					...
					<td>
						...
						<input type="hidden" name="id" value="${v.getId()}" class="form-input">
							<div class="form-group d-flex justify-content-between">
								<a class="btn bnt-sm btn-incre" href="quantity-inc-dec?action=inc&id=${v.getId()}"><i class="fas fa-plus-square"></i></a> 
								<input type="text" name="quantity" class="form-control"  value="${v.getQuantity()}" readonly> 
								<a class="btn btn-sm btn-decre" href="quantity-inc-dec?action=dec&id=${v.getId()}"><i class="fas fa-minus-square"></i></a>
							</div>
							<button type="submit" class="btn btn-primary btn-sm">Buy</button>
						</form>
					</td>
					<td><a href="remove-from-cart?id=${v.getId()}" class="btn btn-sm btn-danger">Remove</a></td>
				</tr>
			</d:forEach>
			</tbody>
		</table>
